...
app.use(session({secret: crypto.randomBytes(16).toString("base64"), resave: false, saveUninitalized: true}));
...
/* Object Constructors */
function Image(author, title, imageData) {
    this.author = author;
    this.title = title;
...
app.post("/signup/", function (req, res, next) {
    ...
    users.findOne({_id: username}, function(err, user) {
        if (err) {
            ...
            users.update({_id: username}, new User(username, hash.digest("base64"), salt), {upsert: true}, function(err) {
                if (err) {
                    return res.status(500).end(err);
                }
                else {
...
app.post('/api/images/', isAuthenticated, upload.single("picture"), function (req, res, next) {
    ...
    if (!req.body.title || !req.file) {
        return res.status(400).end("A required field is missing, please fix request and try again.");
    }
    images.insert(new Image(req.username, req.body.title, req.file), function(err, item) {
        if (err) {
            return res.status(500).end(err);
        }
        else {
...
app.post("/api/comments/:id/", isAuthenticated, function (req, res, next) {
    ...
    images.findOne({_id: req.params.id}, function (err, item) {
        if (err) {
            ...
            return res.status(404).end("No image with id " + req.params.id + " could be found");
        }
        else {
            comments.insert(new Comment(req.username, req.body.content, req.params.id), function(err, item) {
                if (err) {
                    return res.status(500).end(err);
                }
                else {
...
app.get("/api/users/", isAuthenticated, function(req, res, next) {
    users.find({}, {password: 0, salt: 0}).sort({_id: req.username}).exec(function(err, items) {
        if (err) {
            return res.status(500).end(err);
        }
        else {
...
app.get("/api/images/:author", isAuthenticated, function (req, res, next) {
    images.find({author: req.params.author}).sort({createdAt:-1}).exec(function(err, items) {
        if (err) {
            return res.status(500).end(err);
        }
...
app.get("/api/images/:id/:action/", isAuthenticated, function (req, res, next) {
    images.findOne({_id: req.params.id}, function(err, item) {
        if (err) {
            ...
            images.find({author: item.author}).sort({createdAt:-1}).exec(function(err, items) {
                if (err) {
                    return res.status(500).end(err);
                }
                else {
                    ...
                    if (req.params.action === "next") {
                        ...
                        return res.status(400).end("Incorrect action, please fix request and try again");
                    }
...
app.delete("/api/images/:id/", isAuthenticated, function (req, res, next) {
    images.findOne({_id: req.params.id}, function (err, image) {
        if (err) {
            ...
            images.find({author: image.author}).sort({createdAt:-1}).exec(function(err, items) {
                if (err) {
                    return res.status(500).end(err);
                }
                else {
                    ...
                    if (req.username === items[index].author) {
                        images.remove({_id: req.params.id}, function (err, num) {
                            ...
                            comments.remove({imageId: req.params.id}, function(err, num) {
                                let imageIndex;
...
app.delete("/api/comments/:id/", isAuthenticated, function (req, res, next) {
    comments.findOne({_id: req.params.id}, function(err, item) {
        if (err) {
            ...
            return res.status(404).end("No comment with id " + req.params.id + " could be found");
        }
        else {
            images.findOne({_id: item.imageId}, function(err, image) {
                if (err) {
                    ...
                    comments.find({imageId: item.imageId}).sort({createdAt:-1}).exec(function(err, items) {
                        if (err) {
                            return res.status(500).end(err);
                        }
                        else {            
                            // Determine if authenticated action
                            if (item.author === req.username || image.author === req.username) {
                                
                                // Need to determine the page number of the comment page to display once the comment
                                // has been deleted
                                let commentIndex = items.findIndex(x => x._id === req.params.id);
