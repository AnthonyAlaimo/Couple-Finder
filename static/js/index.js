...
let index = (function() {
    ...
    window.onload = function() {
        ...
        let imageFormToggleButton = document.querySelector("#toggle_add_image_form");
        ...
        addCommentForm.addEventListener("submit", (e) => onAddComment(e, currentImageId));
    ...
    function onUserUpdated(e) {
        ...
        document.querySelector("#add_image_form").style.visibility = (username)? 'visible' : 'hidden';
        document.querySelector("#user_select_box").style.visibility = (username)? "visible" : "hidden";
        ...
        if (username) {
            document.querySelector("#add_image_form").style.display = username === api.getUsername() ? 'flex' : 'none';
    ...
    function onImageRequested(e) {
        ...
        if (image) {
            ...
            prevArrow.addEventListener("click", () => api.getLinkedImage(image._id, "prev"));
            if (!image.hasPrev) {
            ...
            nextArrow.addEventListener("click", () => api.getLinkedImage(image._id, "next"));
            if (!image.hasNext) {
    ...
    function onCommentPageRequested(e, comments) {
        ...
        if (items.length > 0) {
            ...
            let prevArrow = document.createElement("div");
            prevArrow.className = "button_left icon";
            prevArrow.addEventListener("click", () => api.getCommentPageForImage(items[0].imageId, comments.page - 1));
            if (comments.page < 1) {
                prevArrow.style.visibility = "hidden";
            }
            ...
            items.forEach(comment => {
                ...
                let deleteButton = document.createElement("div");
                deleteButton.className = "button_delete icon";
                deleteButton.addEventListener("click", () => api.deleteComment(comment._id));
            ...
            commentsContainer.appendChild(commentsPanel);
             ...
             let nextArrow = document.createElement("div");
             nextArrow.className = "button_right icon";
             nextArrow.addEventListener("click", () => api.getCommentPageForImage(items[0].imageId, comments.page + 1));
             if (!comments.hasNext) {
                 nextArrow.style.visibility = "hidden";
             }
