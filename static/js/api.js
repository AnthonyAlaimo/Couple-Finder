...
let api = (function() {
    ...
    ****************************** */ 
    ...
    module.signup = function(username, password) {
        send("POST", "/signup/", {username: username, password: password}, function(err, item) {
            if (err) {
    ...
    module.signin = function(username, password) {
        send("POST", "/signin/", {username: username, password: password}, function(err, item) {
            if (err) {
                notifyErrorHandlers(err);
            }
            else {
    ...
    module.addImage = function(title, imageFile) {
        sendFiles("POST", "/api/images/", {title: title, picture: imageFile}, function(err, item) {
            if (err) {
                notifyErrorHandlers(err);
            }
            else {
    ...
    module.getLinkedImage = function(imageId, action) {
        send("GET", "/api/images/" + imageId + "/" + action + "/", {}, function(err, item) {
            if (err) {
                notifyErrorHandlers(err);
            }
            else {
    ...
    module.addComment = function(imageId, content) {        
        send("POST", "/api/comments/" + imageId + "/", {content: content}, function(err, item) {
            if (err) {
                notifyErrorHandlers(err);
            }
            else {
    ...
    module.getCommentPageForImage = function(imageId, page) {
        send("GET", "/api/comments/" + imageId + "/?page=" + page + "/", {}, function(err, items) {
            if (err) {
                notifyErrorHandlers(err);
            }
            else {
    ...
    module.deleteComment = function(commentId) {
        send("DELETE", "/api/comments/" + commentId + "/", {}, function(err, items) {
            if (err) {
                notifyErrorHandlers(err); 
            }
            else {
